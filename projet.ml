...
(*Dans cette fonction de simplification, il y a 4 cas possible pour simplifier :
  ...
  -  une sous-expression constituÈe exclusivement de constantes.

  -  une expression de la forme 1*x ou 0 + x sera simplifiÈe en x ; de mÍme l'expression 0 * x
     sera simplifiÈ par 0

  -  une sous-expression de la forme x * x sera simplifiÈe en 0 

  -  une sous-expression de la forme x/x sera simplifiÈe en 1
 *)
...
(*Dans cette fonction d'affichage, il faudra afficher l'expression avec le moins de parenthËses
 possible.
 
  -  Ne pas mettre de parenthËse lorsqu'il y a associativitÈ : ((a*b)*c)*(e+f) doit Ítre a*b*c*(e+f)
 *)
